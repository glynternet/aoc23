...
.664.598*.`
	...
	lines := strings.Split(prodInput, "\n")
	lineParts := make([][]partNum, len(lines))
	var symbols [][2]int
	for lineNum, line := range lines {
		...
		for i, char := range line {
			if unicode.IsDigit(char) {
				...
				if num {
					curr := lineParts[lineNum][len(lineParts[lineNum])-1].num
					curr *= 10
					curr += int64(v)
				...
				symbols = append(symbols, [2]int{lineNum, i})
			}
		}
		...
		//		break
		//	}
		//	firstNonDigit := strings.IndexFunc(line[firstDigit:], func(r rune) bool {
		//		return !unicode.IsDigit(r)
		//	})
		//	lastDigit := len(line)
		//	if firstNonDigit != -1 {
		//		lastDigit = firstNonDigit + firstDigit
		//	}
		//	fmt.Println(line)
		//	partNumChars := line[firstDigit:lastDigit]
		//	partNum, err := strconv.Atoi(partNumChars)
		//	require.NoError(t, err)
		//
		//	fmt.Println(lineNum, partNum)
		//	line = line[firstNonDigit+firstDigit+len(partNumChars):]
		//}
	}
